...
public class HelpMessages{
	...
	public static void sendUpdate(Player p, TeamSpeak pl){
		if (p.hasPermission("ts.update") || p.isOp()){
			if (pl.updater.getResult() == Updater.UpdateResult.UPDATE_AVAILABLE){
				...
				p.sendMessage("");
				p.sendMessage(
						ChatColor.translateAlternateColorCodes('&', "&4&l" + cfg.getString("messages.update-info")));
	...
	public static void sendCache(Player p, TeamSpeak pl){
		if (p.isOp() || p.hasPermission("ts.cache")){
			...
			{
				JsonMessage jm = new JsonMessage();
				jm.append(ChatColor.translateAlternateColorCodes('&', cfg.getString("messages.cachenew"))).save()
						.send(p);
			}
			prefixsend(p);
		}
	...
	private static void prefixsend(Player p){
		p.sendMessage(ChatColor.YELLOW + "[]================" + ChatColor.GOLD + ChatColor.BOLD + " TeamSpeak "
